...
def crf_log_loss(y_true, prob_pred):
    '''[summary]

    Arguments:
        y_true {nested list} -- list of true labels e.g. [['O', 'I', 'O'], [...], ...]
        prob_pred {[type]} -- [description]
    '''

    y_true_merged = list(itertools.chain(*y_true))
    prob_pred_merged = list(itertools.chain(*prob_pred))

    y_pred_list = [[y_pred['I'], y_pred['O']] for y_pred in prob_pred_merged]
    return log_loss(y_true_merged, y_pred_list, labels=['I', 'O'])
...
def print_classification_report(y_true, y_pred):
    ...
    Arguments:
        y_true {list} -- list of true labels e.g. [['O', 'I', 'O'], [...], ...]
        y_pred {list} -- list of predicted labels e.g. [['O', 'I', 'O'], [...], ...]
    '''
